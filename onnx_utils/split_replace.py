#! /usr/bin/env python
"""
Script to generate onnx model for split operator.

Usage:
    python split_replace.py -ds 1 16 8 -dt float32 -a 2 -n 4

    -ds: data_shape
    -dt: data_type
    -a: split_axis
    -n: split_number_of_after_division
...
class Model(nn.Module):
    ...
    def forward(self, x):
        result_splited_list = []
        split_start_idx_list = list(
            range(0, x.shape[self.split_axis], self.split_number_of_after_division)
        )
        ...
        if self.split_axis == 0:
            ...
            for split_start_idx in split_start_idx_list:
                split_end_idx = split_start_idx + self.split_number_of_after_division
                result_splited_list.append(
                    x[:, :, :, split_start_idx:split_end_idx, ...]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--opset", type=int, default=11, help="onnx opset")
    parser.add_argument(
        "-ms",
        "--model_name_suffix",
        type=int,
